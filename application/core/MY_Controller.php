...
class MY_Controller extends CI_Controller {
	...
	public function get_user_data(){
		...
		if ($this->db->table_exists('addons_list')):
			$addons = $this->admin_m->select('addons_list');
			if(isset($addons) && count($addons)>0):
				foreach ($addons as $key => $value) {
					if(isset($value['activated_date']) && !empty($value['activated_date'])):
						if($value['activated_date'] < d_time()):
							$check = $this->upload_m->verify($value['purchase_code']);
							if($check==0):
								$data = [
									'active_code' => '',
									'active_key' => '',
								...
								$this->admin_m->update($data,$value['id'],'addons_list');
							elseif($value['script_purchase_code'] != $this->settings['purchase_code']):
								$data = [
									'active_code' => '',
									'active_key' => '',
									'is_install' => 0,
