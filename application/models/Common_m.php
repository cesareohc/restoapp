...
public function count_total_user()
{
	...
	**/
	...
	{
		$this->db->select('or.id,or.uid,or.customer_id,or.customer_rating,or.customer_review,or.rating_time,st.id as customer_id,st.name as customer_name');
    $this->db->from('order_user_list or');
		$this->db->join('staff_list st','st.id = or.customer_id','LEFT');
		if(isset($_GET['sort'])):
				$sort = $_GET['sort'];
				if($sort=='newest'){
	...
	**/
    ...
    {
        $this->db->select('rl.*,rl.id as shop_id,rl.name as shop_name,u.id as user_id,u.username,c.*,c.id as country_id,c.name as country_name');
    ...
    public function get_my_coupons($shop_id)
    {
     ...
     public function check_latest_order($shop_id,$order_type,$email,$phone)
    {
    	$today = today();
