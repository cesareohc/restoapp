		    	...
		    	<div class="userMenu_flex">
			    	...
			    	<?php if(is_feature($id,'welcome')==1 && is_active($id,'welcome')): ?>
				      ...
				      <li class="nav-item <?= isset($page_title) && $page_title=="Specialties"?"active":"" ;?>">
				        <a class="nav-link" href="<?= base_url('specialties/'.$slug) ;?>"><?= get_features_name('specialities');?></a>
				      </li>
				  	<?php endif;?>
				      
			    	</ul>
			    	<div class="rightMenu dis_none">
				    	<ul>
				    		<li class="cart navCart"><a class="nav-link" href="javascript:;"><i class="icofont-cart-alt fa-2x"></i> <span class="cart_count total_count"><?= $this->cart->total_items() ;?></span></a></li>
				    	</ul>
			    	</div>
			    </div>
		    </div>
 ...
 <div class="UserResponsive_menu">
 	<div class="UserMobileMenu">
 		<ul>
	 		...
	 		<?php if(is_feature($id,'specialities')==1 && is_active($id,'specialities')): ?>
	 			<li data-toggle="tooltip" title="Specialties" class="<?= isset($page_title) && $page_title=="Specialties"?"active":"" ;?>"><a href="<?= base_url('specialties/'.$slug) ;?>"><i class="icofont-touch"></i></a></li>
	 		<?php endif;?>
					 	...
					 	<?php else: ?>
					 		...
					 		<div class="bottom_cart_order">
					 			<div class="sub_total_list">
					 				<h4><?= lang('total'); ?>: <?= lang('qty'); ?> <span class="cart_count"><?= $this->cart->total_items();?></span> =  <?= lang('price'); ?> <span class="total_price"><?= number_format($this->cart->total(),2)  ;?></span> <?= shop($shop['id'])->icon;?> </h4>
					 			</div>
					 			<?php  if($user['theme']==1 || $user['theme']==3):?>
					 				<a href="<?= base_url('checkout/'.$slug) ;?>" class="btn btn-info btn-block order-btn"><?= !empty(lang('checkout'))?lang('checkout'):'Checkout'  ;?></a>
					 			<?php else: ?>
						 			<a href="javascript:;" class="btn btn-info btn-block order-btn show_order_btn"><?= !empty(lang('checkout'))?lang('checkout'):'Checkout'  ;?></a>
						 		<?php endif;?>
					 		</div>
					 	<?php endif;?>
