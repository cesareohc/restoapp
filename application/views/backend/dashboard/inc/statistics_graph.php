							...
							<ul>
								...
								<?php foreach ($order_type_list as $key=> $order): ?>
									<li class="hColor_<?= $key+1;?>">
										<h3><?= currency_position($this->statistics_m->get_daily_statistics($_ENV['ID'],$order['id'])->amount,$_ENV['ID']);?></h3>
										<p> <?= $order['type_name'];?> (<?= $this->statistics_m->get_daily_statistics($_ENV['ID'],$order['id'])->count;?>)</p>
									</li>
								<?php endforeach ?>
							</ul>
						</div>
            	...
            	<?php foreach ($order_type_list as  $order_type):?>
            		{
	            		...
	            		y: <?= $this->statistics_m->get_daily_statistics($_ENV['ID'],$order_type['id'])->amount;?>,
	            		drilldown: null  
            		},
            	<?php endforeach;?>	
