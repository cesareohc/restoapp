			...
			<?php endif; ?>
				...
				<?php foreach ($order['item_list'] as $key => $row): ?>
					<?php if($row['is_package']==1): ?>
						...
						$grand_total = @number_formats(grand_total($total_price,$order['delivery_charge'],$order['discount'],$order['tax_fee'],$order['coupon_percent'],$order['tips'],$order['order_type'],restaurant()->tax_status,$order['is_pos']),restaurant()->number_formats);
