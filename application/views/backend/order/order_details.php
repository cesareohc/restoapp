						...
						<?php if($order['order_type']!=7): ?>
							<div class="row ">
									...
									<?php if(isset($_GET['action']) && $_GET['action']=="Edit"): ?>
										<a class="btn btn-secondary" href="#itemModal" data-toggle="modal">
									...
									<?php else: ?>
										<?php if(isset($apps->edit_order_type) && $apps->edit_order_type==1): ?>
												 <?php if(file_exists(APPPATH.'controllers/admin/Pos.php')): ?>
													<a class="btn bg-info-light-active" href="<?= base_url("admin/restaurant/edit_order/{$order_details['uid']}"); ?>"> <i class="mr-1 icofont-ui-edit text-120 w-2"></i>  <?= !empty(lang('edit_order'))?lang('edit_order'):"Edit Order" ;?>  </a>
												<?php endif; ?>
										<?php else: ?>
											 <a class="btn btn-info" href="<?= base_url("admin/order-details/{$order_details['uid']}?action=Edit"); ?>">
									...
									<a class="btn btn-success" target="blank" href="<?= base_url('invoice/'.auth('username').'/'.$order['uid']); ?>" class="btn bg-white btn-light mx-1px text-95"  data-title="PDF">
						...
						<div class="order_information pt-5">
							...
							<div class="mt-20 orderBtnList">
								<?php if($order['status']==0): ?>
									...
									<?php else: ?>
										<a href="javascript:;" class="btn primary-light-active" data-toggle="tooltip" title="Completed"><i class="fa fa-check"></i> &nbsp; <?= lang('paid'); ?> </a> &nbsp;
									<?php endif ?>
