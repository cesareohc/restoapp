							...
							<table class="table table-bordered table-condensed table-striped" id="">
								<thead>
									<tr>
										...
										<th width=""><?= !empty(lang('payment_status'))?lang('payment_status'):"Payment Status";?></th>
										<th width=""><?= !empty(lang('payment_by'))?lang('payment_by'):"Payment by";?></th>
										<th width=""><?= !empty(lang('payment_date'))?lang('payment_date'):"Payment Date";?></th>
								...
								<?php foreach ($payment_history as $key => $row): ?>
									<tr>
										...
										<td>
											<?php if($row['status']=='succeeded' || $row['status']=='Completed' || $row['status']=='Authorized'|| $row['status']=='captured' || $row['status']=='successful' || $row['status']=='approved'): ?>
