...
<div class="payment_content text-center">
	...
	<?php if(is_demo()==0): ?>
		<a href="javascript:;" data-key="<?= !empty($setting['razorpay_key_id'])?$setting['razorpay_key_id']:'' ;?>" data-amount="<?= isset($package['price'])?html_escape($package['price']):'';?>" data-id="<?= isset($package['id'])?html_escape($package['id']):'';?>" data-name="<?=  $u_info['username'];?>" class="btn btn-success buy_now"><?= !empty(lang('pay_now'))?lang('pay_now'):"Pay Now"?> &nbsp;( <?= get_currency('icon');?> <?= isset($package['price'])?html_escape($package['price']):'';?> )</a>
	<?php endif;?>
</div><!-- payment_content -->
<?php else: ?>
	<div class="payment_content text-center">
