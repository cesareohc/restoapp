...
<div class="payment_content text-center <?= is_package;?>">
	...
	<?php if(is_demo()==0): ?>
		...
		<button type="submit" id="blinkCheckoutPayment" name="submit" class="btn btn-success buy_now"><?= !empty(lang('pay_now'))?lang('pay_now'):"Pay Now"?> &nbsp;( <?= get_currency('icon');?> <?= isset($package['price'])?html_escape($package['price']):'';?> )</button>
	<?php endif;?>
</div><!-- payment_content -->
...
<script>
	...
	document.getElementById("blinkCheckoutPayment").addEventListener("click", function(){
		openBlinkCheckoutPopup('<?= $paytm_init['order_id'];?>','<?php echo $paytm_init['token']?>','<?php echo $package['price']?>');
		}
	);
