						...
						<div class="form-group col-md-6">
							...
							<select name="cat_id" class="form-control" id="category" required>
								...
								<?php foreach ($menu_type as $key => $type): ?>
									<option data-type="<?= html_escape($type['type']) ;?>" <?= isset($data['cat_id']) && $data['cat_id']==$type['id']?"selected":"";?> <?= isset($cat_id) && $cat_id==md5($type['id'])?"selected":"";?> value="<?= $type['id'];?>"><?= $type['name'];?></option>
								<?php endforeach ?>
							</select>
						</div>
						<div class="form-group col-md-6">
