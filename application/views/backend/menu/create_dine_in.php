        ...
        <div class="">
			<div class="single_alert alert alert-info alert-dismissible">
				...
				<div class="d_flex_alert ">
					...
					<div class="double_text">
						<div class="text-left">
							<h5><?= !empty(lang('qr_code_generate_msg'))?lang('qr_code_generate_msg'):"After generate Qr code download the Qr code and add in you custom  flyer."  ;?> </h5>
						</div>
	...
	<div class="col-md-8 col-sm-8 col-xs-12 col-lg-6">
		<div class="box box-primary">
			<div class="box-header with-border">
				<h3 class="box-title"><?= !empty(lang('create_qr'))?lang('create_qr'):"Create QR";?> </h3>
			...
			<!-- /.box-header -->
			<form action="<?= base_url('admin/menu/add_dine_in') ?>" method="post" class="skill_form" enctype= "multipart/form-data">
				<div class="box-body">
					...
					<div class="row">
						<div class="form-group col-md-12">
					      	<label for="type_name"><?= !empty(lang('menu_name'))?lang('menu_name'):"Menu Name";?>*</label>
					...
					<div class="row">
						<div class="form-gorup col-md-12 mb-5">
							<label for="price"><?= !empty(lang('price'))?lang('price'):"Price";?> *&nbsp; &nbsp;
					      	 	<label data-toggle="tooltip" data-placement="top" title="If you want to set custom Price" class="pointer p-5 btn label-info discount_label label_input mr-15"><input type="checkbox" name="is_price" class="is_price" value="1" <?= isset($data['is_price']) && $data['is_price']==1?'checked':''; ;?>> &nbsp; Is Price</label> 
					...
					<div class="row">
						<div class="form-group col-md-6">
					      	<label for="type_name"><?= !empty(lang('table'))?lang('table'):"table";?> *</label>
					      	<select name="table_no" id="table_no" class="form-control" required="">
					      		<option value=""><?=  lang('select');?></option>
					      		<?php foreach ($table_list as $key => $table): ?>
					      			<option value="<?= $table['id'] ;?>" <?= isset($data['table_no']) && $data['table_no']==$table['id']?"selected":"" ;?>><?= $table['name'] ;?></option>
					      		<?php endforeach ?>
					      	</select>
				...
				<div class="box-footer">
					<div class="pull-left">
						<input type="hidden" name="id" value="<?= isset($data['id']) && $data['id'] !=0?$data['id']:0 ?>">
		          		<a href="<?= base_url('admin/menu/dine_in'); ?>" class="btn btn-default btn-block btn-flat"><?= !empty(lang('cancel'))?lang('cancel'):"cancel";?></a>
		          	</div>
		          	<?php if(isset($active) && $active==1): ?>
			          	<div class="pull-right">
			          		<button type="submit" name="register" class="btn btn-primary btn-block btn-lg btn-flat"><?= !empty(lang('generate_qr_code'))?lang('generate_qr_code'):"Generate QR code";?></button>
			          	</div>
		          	<?php endif;?>
				</div>
			</form>
