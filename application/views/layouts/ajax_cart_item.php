...
<?php foreach ($this->cart->contents() as $key => $row): ?>
	...
	<?php if($shop_id==$row['shop_id'] && !isset($row['is_pos'])): ?>
	<li>
		<div class="single_item_cart">
			...
			<div class="left_cart">
				...
				<div class="single_cart_item_details">
					...
					<p><?=  currency_position($row['price'],$row['shop_id']).' x '.html_escape($row['qty']) ;?> = <?=currency_position($this->cart->format_number($row['price']*$row['qty']),$row['shop_id']);?></p>
