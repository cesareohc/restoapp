...
protected function order_completed($id,$order_info)
{
	...
	if(isset($sms->is_complete_sms) && $sms->is_complete_sms==1):
		$get_data = $this->get_msg($order_info);
		$complete_data = ['phone' => $get_data['phone'], 'text' => create_msg($get_data['replace_data'],$replace_data['complete_message'])];
...
public function add_configuration()
{
	...
	if ($this->form_validation->run() == FALSE) {
		...
		$is_area_delivery = $this->input->post('is_area_delivery',TRUE);
		...
		$data = array(
			...
			'is_area_delivery' => isset($is_area_delivery)?1:0,
