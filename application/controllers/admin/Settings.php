...
class Settings extends MY_Controller{
	...
	**/ 
	public function add_settings(){
		...
		$this->form_validation->set_rules('long_description', 'Description', 'trim|xss_clean');
		if ($this->form_validation->run() == FALSE) {
			$this->session->set_flashdata('error', validation_errors());
	...
	public function add_payment_settings(){
		...
		if ($this->form_validation->run() == FALSE) {
			...
			redirect(base_url('admin/settings/payment_settings'));
				...
				$offline = array(
					'offline_details' => $this->input->post('offline_details',TRUE),
...
public function upload_pwa_logo()
{
	...
	public function add_notification(){
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$data = array(
				'notifications' => json_encode($onsignal),
			);
			$id = $this->input->post('id');
			if($id==0){
				$insert = $this->admin_m->insert($data,'settings');
